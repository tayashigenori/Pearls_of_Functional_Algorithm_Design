...
boxs = map ungroup . ungroup . map cols . _group . map _group

_group    :: [a] -> [[a]]
_group [] = []
_group xs = take 3 xs: _group (drop 3 xs)
...
expand1 rows = [rows1
                ++ [row1 ++ [c]: row2]
                ++ rows2 | c <- cs]
  where (rows1, row: rows2) = _break (any smallest) rows
        (row1,  cs:  row2)  = _break smallest row
        smallest cs         = length cs == n
        n                   = minimum (counts rows)
counts = filter (/= 1) . map length . concat

_break p xs = (takeWhile (not . p) xs, dropWhile (not . p) xs)
